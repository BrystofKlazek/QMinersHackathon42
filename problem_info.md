...
### Inkrementální data
Inkrementální data obsahují výsledky a statistiky, které jste doposud neviděli. Tato data jsou složena ze dvou tabulek (DataFrames). [Games DataFrame](#games-dataframe) obsahující informace o parametrech odehraných zápasů. Počítejte s tím, že první inkrement který uvidíte, bude obsahovat i zápasy staršího data (z předcházejících sezón). 
